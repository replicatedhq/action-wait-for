...
async function run(): Promise<void> {
  try {
    ...
    let filter: (el: CheckRun) => boolean;
    if (check_regexp !== "") {
      const regexp = new RegExp(check_regexp);
      filter = (item: CheckRun) => regexp.test(item.name);
    } else {
      filter = () => true;
    }
...
async function poll(
  ...
  { ref, check_name, filter }: PollArguments
): Promise<void> {
  ...
  try {
    ...
    if (incomplete.length === 0) {
      ...
      const goodConclusions: (string | null)[] = ["success", "skipped"];
      ...
      core.info(
        `Waiting for ${incomplete.length} checks to complete: [ ${incomplete
